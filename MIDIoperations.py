...
class MidiWav:
  ...
  def truncate_midi(self, start_second=0, duration=30, id=0, update_path=False,
    ...
    # Hence, setting quantizePost to False to avoid quantisation
    ...
    # Asume that the maximum metronome score is the correct one for all the
...
def print_stream_hierarchy(stream, level=0):
    ...
    for element in stream:
        if isinstance(element, music21.stream.Stream):
            print(f"{indent}{element.__class__.__name__} (offset: {element.offset}, duration: {element.duration.quarterLength})")
            ...
            print(f"{indent}Note (pitch: {element.pitch}, offset: {element.offset}, duration: {element.duration.quarterLength}{tie_info}{velocity_info}{lyric_info})")
            ...
            print(f"{indent}Chord (pitches: {pitches}, offset: {element.offset}, duration: {element.duration.quarterLength}{velocity_info}{root_info}{bass_info})")
            ...
            print(f"{indent}MetronomeMark (offset: {element.offset}, duration: {element.duration.quarterLength}{number_info}{text_info})")
        else:
            print(f"{indent}{element.__class__.__name__} (offset: {element.offset}, duration: {element.duration.quarterLength})")